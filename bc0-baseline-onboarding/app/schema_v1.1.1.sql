-- ============================================================
-- BC0 Onboarding — PostgreSQL-Schema (mandantenfähig)  ·  v1.1.1
...
-- Stand: 07.08.2026 · Autor: Simeon Ehmer · Postgres >= 15
--
-- Änderungen ggü. v1.1 (07.08.2026 — rein dokumentarischer Nachzug, keine Datenänderung):
--   [7] Abschnitt 4a ergänzt: beleg_dokumente + bewertung_belege. Diese Tabellen laufen seit
--       10.07.2026 produktiv (Beleg-Ingestion Stufe 1). Sie wurden von app.py per
--       CREATE TABLE IF NOT EXISTS angelegt und waren bis hierher in keinem Schema-Dokument
--       beschrieben. Aufgefallen beim Ist-Abgleich gegen die Live-Datenbank am 07.08.2026.
--   [8] Schlusszeile korrigiert: Belegablage ist Supabase Storage, nicht MinIO.
--   Die Versionsnummer v1.2 bleibt dem Enrichment-/Provenance-Layer (Abschnitt 8) vorbehalten.
--   ADR-002 gilt unverändert — v1.1.1 ist rein additiv, kein Breaking Change.
--
-- ID-Formate (verbindlich gemäß ADR-002 vom 12.07.2026):
--   Hauptprozess    KP-XX             z. B. KP-02
--   Teilprozess     KP-XX.TP-Y        z. B. KP-02.TP-3      <- Teilprozessteil EINSTELLIG
--   Item-Bewertung  KP-XX.TP-Y.I-NN   z. B. KP-02.TP-3.I-07
--   Ist-Stand geprüft am 07.08.2026: 600 Bewertungen, 0 Abweichler.
--
...
-- ============================================================
-- 4a. BELEG-DOKUMENTE (Beleg-Ingestion Stufe 1, produktiv seit 10.07.2026)
--     Nachgetragen am 07.08.2026 aus app.py (DOC_DDL_PG). Ablage der Dateien:
--     Supabase Storage (Bucket 'belege') oder lokales Volume unter BELEGE_DIR.
--     ref_id traegt eine stabile ID: KP-XX oder KP-XX.TP-Y (siehe Kopf).
-- ============================================================
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS beleg_dokumente (
  ...
  company_id     UUID NOT NULL REFERENCES companies(company_id) ON DELETE CASCADE,
  ref_id         TEXT NOT NULL,           -- stabile ID: KP-XX oder KP-XX.TP-Y
  filename       TEXT NOT NULL,
  storage_key    TEXT NOT NULL,           -- Pfad/Key in Supabase Storage bzw. im Volume
  mime_type      TEXT,
  seiten         INTEGER,
  ocr_text       TEXT,                    -- gefuellt ab Stufe 2 (OCR-Worker, #139)
  ocr_confidence NUMERIC(4,3),
  extrakt        JSONB,                   -- strukturierter Auszug, Stufe 3 (#140)
...
-- Verknuepfung Bewertung <-> Beleg-Dokument (n:m), optional mit Zitat und Seitenzahl.
-- Ergaenzt das Pflichtfeld bitkom_bewertungen.beleg (Freitext) um den Dokumentbezug.
CREATE TABLE IF NOT EXISTS bewertung_belege (
  company_id   UUID        NOT NULL,
  bewertung_id VARCHAR(28) NOT NULL,      -- entspricht bitkom_bewertungen.id
...
-- ============================================================
-- Ende v1.1.1. Projekt-Stack: PostgreSQL (Supabase, Region eu-west-1) + Supabase Storage (Belege).
-- MinIO ist mit der Supabase-Entscheidung vom 12.07.2026 entfallen (ADR-001).
-- ============================================================
