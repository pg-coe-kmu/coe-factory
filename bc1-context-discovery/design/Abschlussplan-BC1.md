...
- **Signatur-Sicht liegt zweimal im Repo** (`prozessprofil.sql` 0b, `sessions.sql` 0b, seit B1 13.09.) — bewusst, solange es zwei Einspiel-Einheiten sind; **bei einer dritten** wird die Sicht in einen Generator gezogen, nicht vorher (YAGNI). `sessions.sql` prüft keine `mitglied|`-Kanten und keine Funktionen — das tut `prozessprofil.sql`, das im Betrieb immer zuerst läuft (`EINSPIELEN.md` §2).
