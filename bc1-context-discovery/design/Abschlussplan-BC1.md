...
| **B6** | **Spalte `step_frequency_per_year` (#255)** | BC2 hat D3 am 20.09. gebunden (Vertrag 1.2, Invariante I8); `lesen.sql` fragt die Spalte ab und brach am Bestand — BC2 las **nichts** (#249/#255). Simeon 21.09.: „Richard baut die Spalte." **GEBAUT 22.09.** (Branch `bc1-d3-spalte`): Spalte numeric im Wertebereichs-CHECK + Sollsignatur neu (177 Zeilen) · Migrations-Einheit `prozessprofil_d3.sql` (M0–M3, läuft vor `prozessprofil.sql`, Nachweis = Fall 2) · Writer liest D3 · Fixture in Betriebsreihenfolge · `EINSPIELEN.md` §2/§11 | Zweitmeinung (Datenmodell) → Live-Lauf `lauf.sh d3` (Richard) → `lesen.sql` an der Supabase gegenprüfen → Kommentar in #255 → PR nach `main` | Richard (Live-Lauf), Termin vor Durchstich KW 40 (#206) | S |
