...
2. **B4 — Dienst-Anmeldung an BC0 + Endpunkte** (Zugangsdaten des Anwendungskontos
   liegen seit 22.09. vor — lokal bei Richard, nie im Repo): setzt Zuordnung und Status
   `im_interview` selbst. Reihenfolge bleibt B5 → B4, weil Zuordnung und Status an der
   Anfrage hängen. **Rückfall, falls B4 bis KW 40 nicht fertig ist:** BC0 setzt
   Zuordnung und Status im Durchstich von Hand — ein benannter Handeingriff, kein
   Scheitern.
...
weder Datenmodell noch Verträge noch die Live-Datenbank berühren. Jedes hat ein eigenes
...
|---|---|---|---|---|
| **P1** #267 | README + `CLAUDE.md` auf den Stand nach dem MVP-Kern bringen (Mitarbeit, Arbeitspakete, Struktur) · README-Zusage aus #233: PII-Quoten sind Offline-Wert aus dem Testset, kein Betriebsbestand | A2 | Doku | S |
| **P2** #268 | Dockerfile + `.dockerignore` für den Dienst (kein root-Betrieb, `uv.lock`), README-Absatz | B3 Teil 1 | Konfiguration | S |
| **P3** #269 | Verständliche Fehlertexte im Chat statt roher Schlüssel: `chat_text` neben dem stabilen `detail` | B3 Teil 1 | Code, TDD | S |
| **P4** #270 | Beschädigte Sitzungszeile: `KorrupterStateError` statt rohem `KeyError`/500 | B3 Teil 1 | Code, TDD | S |
| **P5** #271 | Betriebsprotokoll Teil 1: eine Zeile je Turn im Dienst, Correlation-ID, ohne Gesprächsinhalte | B7 Teil 1 | Code, TDD | S–M |
...
2. **B4 direkt nach B5** — der Zugang liegt vor; der Rückfall „von Hand" gilt nur, falls B4 bis KW 40 nicht fertig wird.
...
- **Neu 22.09. — Ticket-Entwürfe Logging/Deployment** (Folie 22.09.: „werden angelegt") sind als Sammel-Issues **#265** (B3) und **#266** (B7) angelegt (22.09.); Philipps Pakete #267–#271 (P1–P5) hängen darunter.
...
| **Durchstich KW 40 ohne B4** | Termin verschieben · BC0 setzt Zuordnung/Status von Hand | **von Hand als benannter Rückfall** — der Durchstich prüft die Strecke, nicht unsere Anmeldung; B4 folgt direkt nach B5 (Zugang seit 22.09. da) | Richard mit Simeon |
...
- **Offen benannt:** Aufwände sind Schätzungen; Hosting ist die eine Entscheidung von
  außen (blockiert B3 Teil 2); der Durchstich hängt an zwei BC0-Antworten und daran,
  dass B5 und B4 bis KW 40 stehen; C3 ist bewusst grob, weil der Bedarf nicht feststeht.
