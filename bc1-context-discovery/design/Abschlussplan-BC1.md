...
| **A7** | **Auswirkungsprüfung v3.2–v3.6 + Gerüst v3.4** | **GEMESSEN 22.09.** — `design/Auswirkungspruefung-BC0-v3.2-v3.6.md`: vier Sichten live wortgleich, Eigner-Index identisch, **nichts bricht**. Gerüst nachgezogen (`aktiv` auf drei Tabellen, `ref_personen` + FK, `ux_prozess_eigner_eindeutig`, `v_teilprozesse_lesen`/`v_systeme_lesen`, `owner_rolle_id`). **Lesepfad nur noch über Sichten** — stillgelegte Teilprozesse/Systeme/Kernprozesse sind draußen (4 Tests). Neu seit A5: `bc1_role` hat kein direktes SELECT auf `prozess_personen` mehr (unser Verzicht 15.09., folgenlos). Branch `bc1-geruest-v36` (über `bc1-d3-spalte`) | Codex-Zweitmeinung → PR nach `main` (nach #263) | — | S |
