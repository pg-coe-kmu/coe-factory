...
> ⛔ **Diese Voraussetzung hält für E4, aber NICHT für die vier Dezimalspalten
> (Task-11-Review 02.09., gemessen — Klärpunkt K-J).** Für `PROZENT_GANZ_0_100` ist die
> Aussage belegt (Rastermessung: `70,5`/`70.5`/`101`/`1.000`/310-stellig alle abgewiesen).
> Für `ZAHL`/`MINUTEN` ist sie widerlegt: ab 309 Stellen liefert die interne
> Float-Normalisierung `None`, der Wert wird **unnormalisiert** durchgereicht, der
> Validator sagt trotzdem `gueltig` (das Muster passt), und `Decimal()` scheitert am
> stehengebliebenen Komma bzw. an den Tausenderpunkten. Selbst gemessen: `"9"*309 + ",25"`
> und `"1" + ".000"*103` sind `gueltig` und nicht konvertierbar; alle realistischen
> Eingaben (`120`, `1.000`, `12.345,5`, `1.234.567,89`, neunstellig) sind sauber. Die
> Wahrscheinlichkeit ist gering, die Folge hart (Session dauerhaft im 503). **Der Satz
> „kein regulärer Nutzerwert kann dort landen" gilt damit nur für E4** — bis K-J
> entschieden ist, nicht als allgemeine Zusicherung lesen.
...
>
> ✅ **ENTSCHIEDEN (Richard, 02.09.): Option (a).** Der S-NN-Sweep läuft VOR der
> Spaltenableitung; die typisierten Spalten werden aus dem bereits bereinigten Payload
> abgeleitet. Die Invariante lautet: **Spalten werden nie aus ungesweepten Werten
> abgeleitet** — sie gehört als Test in die Suite, damit sie bei der nächsten neuen
> Spalte nicht still bricht.
...
5b. **Verbindungsrolle des Dienstes (Task-15-Review, 03.09.):** `main.py` öffnet den
   Profil-Pool ohne `options=-c role=bc1_role`, die Tests setzen die Rolle dagegen
   explizit. Produktiv entscheidet also allein der Rollen-Anteil von `BC1_DB_DSN`, wer
   schreibt — dieselbe offene Frage wie in **K-I** („als welche Rolle verbindet der
   Dienst?"). In EINSPIELEN.md festhalten: DSN-Rolle = `bc1_role`, sonst greifen weder
   die Tabellenrechte noch die Mandantentrennung wie getestet.
...
## Nachtrag 08.09.2026 — Deploy ausgeführt, K-I geschlossen, Anhang A geliefert

**K-I erledigt.** BC0 hat das `ALTER DEFAULT PRIVILEGES` für Schema `bc1` entfernt und
gemessen; wir haben gegengemessen (`pg_default_acl` ohne `bc1`-Zeile). Damit ist das letzte
Deploy-Gate gefallen. Die Defaults für `bc2`/`bc3`/`bc4` bestehen weiter — BC0 hat sie
bewusst stehen lassen und den anderen Kontexten gemeldet.

**Erster Live-Einspiellauf gelungen** (PostgreSQL 17.6): Lauf 1 `Fall 1` + „Sollsignatur
bestaetigt", Lauf 2 `Fall 2`. **Das K-G-Restrisiko hat sich nicht bestätigt** — die im
Container nicht nachstellbare Konstellation (`postgres` als Nicht-Superuser in `bc1_role`)
läuft live sauber durch. Einzelheiten in `bc1_service/db/EINSPIELEN.md`, Abschnitt 9.

**Anhang A ausgeführt** — die drei Use-Case-Testprofile stehen in `bc1.prozessprofil`,
geschrieben über den regulären Weg (Skript → `process_turn` → `ProfilWriter`), je 26/26
Pflichtfelder, `erhebung_id = E-2026-08`, keine offenen Drafts.

> **Gefundener Planungsfehler, ehrlich festgehalten:** Anhang A Kriterium 3 verlangt eine
> per SQL wiederfindbare Testdaten-Kennzeichnung. Der erste Lauf schob sie als **achte**
> Nachricht nach — zu spät: mit dem letzten Pflichtfeld wird der Kern terminal (`fertig`)
> und der Writer friert die Zeile ein, spätere Extraktionen bleiben `fehlt`. Kriterium 5
> hatte genau davor gewarnt. `fertig`-Zeilen sind gegen UPDATE **und** DELETE gesperrt, also
> nicht reparierbar. Entscheidung Richard (08.09.): **Version 2 mit Kennzeichnung schreiben**
> statt Version 1 unkommentiert zu lassen. Ergebnis: je Teilprozess zwei Zeilen; die jüngste
> Version — die BC0/BC2 lesen — trägt die Kennzeichnung. **Lehre für Rev. 12:** optionale
> Kennzeichnungsfelder gehören in dieselbe Nachricht wie das letzte Pflichtfeld.
...
| K-B | Rechte und Leserollen | **BEANTWORTET 02.09.:** GRANTs erteilt, Leser ist `bc_leser`, `profil_write_status` nur `bc1_role`. `SELECT` auf `ref_erhebungen`: **erteilt (gemessen 02.09.)**. **ERLEDIGT 08.09.:** BC0 bestätigt — `bc_leser` gilt für `prozessprofil` **und** `profil_rollen` gleich; `profil_write_status` bleibt allein bei `bc1_role`. Live gegengemessen. | — |
| K-C | Zahlen-Wertebereiche (0 zulässig? Präzision) | **ENTSCHIEDEN (Richard, 03.09.): 0 ist zulässig, Kommastellen sind erlaubt, ganze Zahlen ebenso.** Das ist genau der heutige Stand — `prozessprofil_zahlen_wertebereich` prüft `>= 0` und endlich, der Spaltentyp `numeric` lässt beides zu. **Keine Codeänderung**, der CHECK bleibt wie er ist. BC2 wird informiert (Entscheidung, keine Rückfrage mehr); meldet BC2 später engere Anforderungen, ist ein `ALTER` auf Testdaten vertretbar | nichts mehr — Deploy-Gate K-C ist damit offen |
| K-D | Company-UUID im Snapshot-Export | zugesagt mit BC0-Issue #141 (Termin offen) | nur den Snapshot-Abgleich der Startprüfung |
| K-E (neu) | **BEANTWORTET 08.09.:** BC1 bekommt ein **Anwendungskonto, Rolle `admin`**, dem Mandanten zugeordnet. Anmeldung über `POST /api/auth/login` → **HttpOnly-Sitzungs-Cookie, kein Bearer-Token** (BC0 will keine zweite Anmeldeart; ein Token im `localStorage` wäre genau das Risiko, das das Cookie vermeidet). Unser Dienst muss das Cookie mitführen — **neuer Etappe-2-Punkt**. Konto läuft auf `richard.prezer@gmail.com`, Passwort per SMS. | Etappe 2 (Zuordnungs-/Status-Endpunkt) — nicht diesen Plan |
| K-F (neu) | **BEANTWORTET 08.09.: Zeilen reichen, keine Doku je Use Case.** Begründung BC0: BC2 rechnet aus dem Profil, nicht aus Prosa; Nachvollziehbarkeit trägt ADR-005 R2 (vier Angaben je Wert, stehen im `profil`-JSON); geprüft wird an der Spalte-zu-Feld-Tabelle. BC0-Ersatzwerte im BC1-Schema sind vom Tisch. **Anhang A am 08.09. ausgeführt** — sechs Zeilen (je TP Version 1 ohne, Version 2 mit Testdaten-Kennzeichnung). | — |
| **K-G (Rev. 11b) — ERLEDIGT 03.09.: Ausnahmeliste statt Eigenrollen-Begrenzung.** Zuerst war „nur unsere fünf Rollen" entschieden; beim Umsetzen kam heraus, dass das **`test_mitgliedschaft_in_bc1_role_wird_erkannt` (Codex N10-C2) hätte löschen müssen** — ein `GRANT bc1_role TO irgendwer` wäre unsichtbar geworden. Richard hat daraufhin die schärfere Variante gewählt: Die Signatur betrachtet weiter **alle** Rollen, ausgenommen sind nur **drei namentlich genannte Umgebungsrollen**. **Am 03.09. in der Ziel-Supabase gemessen** (nicht vermutet), welche fremden Rollen überhaupt an unsere Tabellen kämen: `postgres` (Mitglied von `bc1_role` UND `bc_leser`, dort kein Superuser), `supabase_read_only_user` und `supabase_etl_admin` (beide über `pg_read_all_data`) — von 19 Nicht-Superuser-Rollen insgesamt. `supabase_etl_admin` stand in keiner bisherigen Notiz. Umgesetzt als Temp-Tabelle `bc1_umgebungsrollen` in Abschnitt 0b, ausgewertet in `mitglied\|`, `effektiv\|` und `effektiv_spalte\|`. Container-Signatur unverändert (172 Zeilen). **Beide Seiten per Mutation belegt:** Rolle aus der Liste nehmen → Toleranz-Test rot; beliebige Fremdrolle in die Liste setzen → Codex-Test rot | **Geltungsbereich der Sollsignatur vs. Zielumgebung.** Gemessen 02.09.: in der Supabase ist `postgres` kein Superuser, Mitglied von `bc1_role` (Admin-Option) und `bc_leser`; dazu Supabase-Systemrollen (`supabase_read_only_user` via `pg_read_all_data`). Die im Container erzeugte Signatur (`mitglied|`, `effektiv|` über ALLE Rollen) passt damit NICHT zum Ziel → Fall 3 beim Einspielen. Optionen: (a) `effektiv|`/`mitglied|` auf eine normative Rollenmenge begrenzen und Rollen, die ihr Recht nur über `bc1_role`-Mitgliedschaft oder `pg_read_all_data` haben, ausnehmen · (b) Gerüst spiegelt die Supabase (`GRANT bc1_role TO postgres`) — reicht NICHT, weil `postgres` im Container Superuser ist und aus `effektiv|` fällt · (c) Signatur gegen das Ziel erzeugen — braucht ein NICHT-destruktives Werkzeug (`signatur-erzeugen.py` droppt Schemata und darf nie gegen die Supabase laufen). Empfehlung: (a), mit ausdrücklicher Doku, was dann NICHT mehr erkannt wird (Codex N10-C2 wollte genau diese Breite) | Richard, vor Task 16 Step 1 Punkt 5 | Supabase-Deploy, nicht den Bau |
| **K-H (Task-10b-Review) — ERLEDIGT 03.09., Option (a).** Test-Container auf **PostgreSQL 17.11** gehoben, Sollsignatur dort neu erzeugt: **169 → 172 Zeilen, Diff = genau die drei vorhergesagten `acl\|<T>\|bc1_role\|MAINTAIN\|f`**, keine Entfernung, sonst nichts. Suite 445 grün, auch mit `-W error`. Die `docker run`-Befehle in allen fünf Planungsdokumenten stehen auf `postgres:17`; historische Messnotizen „(postgres:16, …)" bleiben unangetastet, sie sind Protokoll. **Korrektur eines eigenen Fehlers:** Der frühere Satz „damit bekommt die Effektiv-Sicht das achte Recht mit" war FALSCH — die Sicht hat eine fest verdrahtete Sieben-Rechte-Liste, die Hauptversion ändert daran nichts. **Nachgemessen (03.09.), warum das trotzdem kein Loch ist:** `MAINTAIN` entsteht nur durch ein direktes `GRANT`, und das fängt die `acl\|`-Prüfung. Erbt eine fremde Rolle von `bc1_role`, erscheint sie schon über die sieben Rechte (+21 Zeilen gemessen), die Abweichung fällt also in jedem Fall auf. Unsichtbar bliebe allein, WELCHE Rolle ein ausschließlich vererbtes `MAINTAIN` hat — die auslösende `acl\|`-Zeile steht trotzdem da. Bewusst nicht geändert (YAGNI, chirurgisch): die Sicht bleibt bei sieben Rechten | **Hauptversion: die Signatur ist PG-16-Text, das Ziel ist PG 17.** Zweiter, von K-G UNABHÄNGIGER Grund, aus dem das Einspielen heute scheitert — beide müssten gelöst sein. PostgreSQL 17 kennt das achte Tabellenrecht `MAINTAIN`; die Eigentümer-ACL lautet dort `arwdDxtm`. **An einem PG-17.11-Container nachgemessen (02.09.), nicht hergeleitet:** Gerüst + DDL laufen durch bis zur Nachprüfung, dann `Fall 3` mit exakt drei Zeilen — `+ zuviel: acl|prozessprofil\|profil_rollen\|profil_write_status\|bc1_role\|MAINTAIN\|f`. Nebenbefund: die `effektiv\|`-Sicht der Signatur hat eine feste Sieben-Privilegien-Liste und ist für `MAINTAIN` blind — ein über Rollenvererbung erlangtes `MAINTAIN` würde sie nicht zeigen (direkt erteiltes schon, über `acl\|`). Optionen: (a) Test-Container auf PG 17 heben und Signatur neu erzeugen (Testumgebung = Zielumgebung, sauberster Weg, ändert die Signatur global und ist deshalb eine eigene Entscheidung) · (b) `MAINTAIN` in Signatur und Effektivprüfung aufnehmen und die Signatur versionsabhängig normalisieren · (c) beim Deploy bewusst gegen die Zielversion neu erzeugen (setzt K-G (c) voraus). Empfehlung: (a) — sonst prüfen wir dauerhaft eine Umgebung, die es nicht gibt. Gefunden von Codex, verifiziert durch eigene Messung | Richard, vor Task 16 | Supabase-Deploy, nicht den Bau |
| **K-J (Task-11-Review) — ENTSCHIEDEN (Richard, 02.09.): Option (a)**, Validator um Endlichkeit und eine Stellen-Obergrenze passend zu `numeric` ergänzen, inklusive Test für beide Varianten; die Semantikänderung an `schema_version` 1.0 ist bewusst in Kauf genommen | **`ZAHL`/`MINUTEN` lassen Werte als `gueltig` durch, die nicht in `numeric` konvertierbar sind.** Die Voraussetzung, unter der Task 11 den harten Fehler überhaupt erlaubt („kein regulärer Nutzerwert kann dort landen"), ist für die vier Dezimalspalten widerlegt. **Ursache, selbst nachgemessen:** `_zahl_in()` gibt bei Float-Überlauf (ab ~309 Stellen) `None` zurück; `_normalisiere_zahl`/`_minuten` reichen den Wert dann UNVERÄNDERT durch, der Validator prüft nur das Muster und sagt `gueltig`, und `Decimal()` scheitert am stehengebliebenen Komma bzw. an den Tausenderpunkten. Belegt mit `"9"*309 + ",25"` und `"1" + ".000"*103`; alle realistischen Eingaben sind sauber. **Folge:** `ProfilWriteError` → laut Plan 503 in JEDEM Turn, der Wert ist zu dem Zeitpunkt schon persistiert → die Sitzung hängt dauerhaft, ohne dass der Nutzer erfährt, welches Feld schuld ist. **Verwandte zweite Variante** (umgeht den Konverter, schlägt erst in Task 14 zu): `"1"*140000` ist `gueltig` und konvertierbar, aber die Spalte lehnt es ab (`NumericValueOutOfRange`) — `ZAHL`/`MINUTEN` haben keine Obergrenze, die Spalte schon. Optionen: (a) `ZAHL`/`MINUTEN`-Validator um Endlichkeit und eine Stellen-Obergrenze passend zu `numeric` ergänzen — der vom Plan selbst vorgeschriebene Weg („erst den Feldtyp an die Spalte angleichen"), **ändert aber die Semantik von `schema_version` 1.0**, also des kontextfreien Zweigs · (b) im Writer abfangen — widerspricht derselben Plan-Regel und behandelt das Symptom · (c) bewusst so lassen und die Plan-Zusicherung auf E4 einschränken (bereits geschehen, siehe Task 11). Empfehlung: (a), zusammen mit einem Test für beide Varianten. Gefunden im unabhängigen Review, durch eigene Messung bestätigt | Richard, vor Task 14 | Task 11 ist unverändert korrekt; betrifft die Feldtypen im Kern |
| **K-I (neu, Task-10b-Review, DSGVO)** | **`bc1.sessions` leckt im Produktivbetrieb an BC2/BC3/BC4.** `PostgresStateStore` legt die Session-Tabelle selbst an (`postgres_store.py` Z. 18), außerhalb von `prozessprofil.sql` und damit ohne jedes `REVOKE`. BC0s `ALTER DEFAULT PRIVILEGES FOR ROLE bc1_role IN SCHEMA bc1` greift bei JEDER Tabelle, die `bc1_role` dort anlegt. **Am Container im Produktivfall nachgestellt (02.09.):** als `bc1_role` angelegt → `relacl = {bc_leser=r/bc1_role, bc1_role=arwdDxt/bc1_role}`, alle vier fremden Rollen haben SELECT. Die Tabelle enthält den vollständigen Interview-Verlauf inklusive Nutzer-Rohtext. **Kein Test sieht es, und zwar systematisch:** im Testlauf verbindet der Store als `postgres`, die Tabelle gehört dann `postgres`, und das Default-Privileg gilt `FOR ROLE bc1_role` — die Fixture verdeckt den Produktivfall. Nicht von Task 10b verursacht (die Lücke besteht seit dem Store), aber hier zum ersten Mal sichtbar, weil hier bewusst entschieden wird, wer in `bc1` liest. Optionen: (a) BC0 bitten, das `ALTER DEFAULT PRIVILEGES` für Schema `bc1` zu entfernen — BC1 vergibt seine Leserechte dann selbst und ausdrücklich (Ursache, nicht Symptom) · (b) `REVOKE ALL ON bc1.sessions FROM bc_leser` im Store nachziehen (Notnagel, wirkt nur für diese eine Tabelle) · (c) beides. Dazu ein Test, der die Tabelle **als `bc1_role`** anlegt und den Lesezugriff verneint. Offene Vorfrage: als welche Rolle verbindet der Dienst produktiv (`BC1_DB_DSN`)? Gefunden im unabhängigen Review, verifiziert durch eigene Messung | Richard, vor dem ersten Produktivlauf | Etappe 2 / Deploy, nicht diesen Plan |
| **Notiz (03.09., Richard): unbewertete Teilprozesse sind KEIN Projektfall.** Die drei Use Cases sind die Automatisierungsbeispiele des Hochschulprojekts, und sie sind vollständig bewertet — in der Supabase gemessen: `KP-05.TP-1`, `KP-06.TP-1`, `KP-06.TP-2` je 30/30 Items in `E-2026-08`. Der Guard aus Task 10b („nur bewertete TPs interviewbar") bleibt — er ist billig, getestet und richtig. **Formulierung gegenüber BC0 (Richard, 03.09.): als Voraussetzung, nicht als Frage** — „Ein Teilprozess muss vollständig bewertet sein, bevor BC1 ihn interviewt; das Self-Rating ist Eingang für BC1, nicht Ergebnis des Interviews." So steht es im PDF vom 03.09. und so gehört es in die Betriebsdoku (Task 16) und in den BC0→BC1-Vertrag. Die Produktfrage „wer füllt das Self-Rating für die übrigen 27 von 50?" liegt außerhalb des Projektumfangs — Roadmap-Anker für ein Produkt, keine Agenda | — | Task 16 (Voraussetzung dokumentieren) | nichts |
| **K-K (Task-13-Review) — durch Messung 03.09. praktisch entschieden: Option (a), so lassen.** Die Bewertungen der drei Use Cases liegen genau in der OFFENEN `E-2026-08`; nur `abgeschlossen` zu binden würde die Use Cases vom Interview ausschließen. BC0 wird im PDF vom 03.09. um Bestätigung gebeten und darum, `E-2026-08` nicht zu verwerfen (abschließen ist unschädlich) | **Gilt eine OFFENE Erhebung als „aktuell“?** `v_bewertung_aktuell` schließt nur `verworfen` aus, also gewinnt eine laufende Erhebung, sobald sie den jüngsten Stand hat — gemessen an einer Fixture-Zeile mit Status `offen`. Folge: das Profil bindet seine NOT-NULL-`erhebung_id` an eine Erhebung, die BC0 später auf `verworfen` setzen kann; der FK auf `ref_erhebungen` kennt den Status nicht und hält das nicht auf. In der Supabase ist der Fall real: NoroAI hat E-2026-06 (abgeschlossen 07.08.) **und** E-2026-08 (offen 24.08.). BC0-Antwort 1a adressiert den Status nicht. Optionen: (a) so lassen — konsistent mit der Sicht, im Docstring festgehalten, BC0 bestätigen lassen (Empfehlung) · (b) nur `abgeschlossen` binden — würde frische Bewertungen ignorieren · (c) BC0 um einen Statuswechsel-Hinweis bitten. **Verhalten ist heute per Test festgenagelt**, die Frage ist nur, ob es das gewollte ist · **NEU aus dem Task-14-Review (gemessen):** Der Schutz greift nur, solange KEIN Draft steht. Im Regelbetrieb steht er ab Turn 1 — der Erhebungs-Lookup laeuft dann nicht mehr, und `_einfrieren` prueft die Erhebung nicht nach. Gemessen: Draft anlegen (`E-2026-02`), danach alle Erhebungen des Mandanten auf `verworfen` setzen, dann abschliessen → Payload geht raus, Zeile steht auf `fertig` mit einer verworfenen `erhebung_id`. Der Testname versprach mehr, als der Code haelt; der Kommentar ist korrigiert. Zusatzoption (d): `_einfrieren` prueft die Erhebung nach — kostet einen Roundtrip je Abschluss und macht aus einem stillen Fehlstand einen 503 | Simeon/BC0 (mit der nächsten Antwort) | nichts — nur Bestätigung, sonst spätere Korrektur |
| **K-L (neu, Task-13-Review)** | **Lesart der `erhebung_id` für BC2.** `v_bewertung_aktuell` ist **itemweise** aktuell: ein Teilprozess kann gleichzeitig aktuelle Bewertungen aus mehreren Erhebungen tragen (gemessen an A/KP-01.TP-1: Item 1 aus E-2026-02, Item 2 aus E-2026-01). Wir speichern genau eine ID — die mit dem jüngsten Stand. Liest BC2 die Bewertungen über `WHERE erhebung_id = profil.erhebung_id` auf `bitkom_bewertungen`, sieht es einen **unvollständigen** Satz; liest es über `v_bewertung_aktuell`, stimmt alles. Bei 30 Bitkom-Items kann der Anteil beliebig klein werden. Ein Vertrag in `contracts/bc1-to-bc2/` existiert noch nicht. Empfehlung: die Spalte im Vertrag ausdrücklich als **„jüngste beteiligte Erhebung (Wasserzeichen), nicht vollständige Provenienz“** definieren; im Docstring steht es bereits | BC2 + Platform (mit dem BC1→BC2-Vertrag) | nichts heute — schlägt bei der Vertragsabnahme zu |
| **Notiz (Task-13-Review, nicht ändern)** | `tests/db_fixture.py` — `verbindung()` setzt `SET ROLE` **innerhalb** der Transaktion. Nach einem erwarteten DB-Fehler plus `rollback()` fällt die Verbindung still auf `postgres` (Superuser) zurück. Task 13 ist nicht betroffen (kein DB-Fehler im Testpfad), aber ein künftiger Test, der nach einem erwarteten Fehler auf derselben Verbindung weitermisst, würde als Superuser messen und still grün sein — genau die K-I-Falle. Vor dem nächsten Rechte-Test prüfen | wir selbst, vor Task 16 | Rechte-Tests ab Task 16 |
| **K-M (neu, beim K-J-Fix gemessen)** | **`normalisiere()` kann einen Wert erzeugen, den der eigene Validator ablehnt.** Gemessen: `ZAHL.normalisiere("0,000001")` → `"1e-06"`, `ZAHL.validator("1e-06")` → `False`. Ursache: `_formatiere()` gibt `str(float)` zurück, und Python schaltet unterhalb von 1e-4 auf Exponentialschreibweise, die `_nur_zahl` nicht kennt. Folge im Betrieb: `status_fuer()` prüft im Extractor den NORMALISIERTEN Wert — eine Eingabe unter 0,0001 wird als ungültig zurückgewiesen, und die Wiederholung derselben Eingabe hilft nicht. **Nicht K-J und nicht gefixt** (chirurgische Änderung: der Fix ändert die Normalisierungssemantik und damit wieder `schema_version`). **Tragweite gering:** betroffen sind nur Werte < 0,0001 in `frequency_per_year`, `executions_per_run` und den beiden Minuten-Spalten — dort fachlich unplausibel; ganzzahlige Riesenwerte sind nicht betroffen (`_formatiere` nimmt für sie den `int`-Zweig, gemessen bis 10^20). Optionen: (a) `_formatiere` auf `Decimal`-Formatierung ohne Exponent umstellen · (b) so lassen und im BC1→BC2-Vertrag festhalten, dass Werte < 0,0001 nicht erfasst werden | Richard, wenn ein Use Case so kleine Zahlen braucht | nichts heute |
| **Notizen aus dem Task-15-Review (bewusst deferiert)** | Fünf Befunde aus dem zweigleisigen Review vom 03.09., jeweils adjudiziert und hier nachgehalten. **(a)** Andere Exceptions als `ProfilWriteError` aus `reconcile` ergeben 500 statt 503 (Codex, Minor). Mit dem Task-14-Writer nicht erreichbar — er wandelt unerwartete Fehler im Terminal-Turn selbst in `ProfilWriteError` um. Nicht gefixt, weil ein breites `except Exception` in der API Programmierfehler verschlucken würde. **Nächster Schritt:** wird je ein anderer Writer eingehängt, den Exception-Vertrag vorher als Protocol festschreiben und mit einem Stub testen. **(b)** Enthielte der LLM-Abschlusstext zufällig denselben festen Hinweis-Wortlaut, stünde er zweimal da (Codex, Minor). Heute unmöglich — die Texte stehen in keinem Prompt. **Nächster Schritt:** falls die Hinweistexte je in den LLM-Kontext wandern, an der Komposition deduplizieren. **(c)** `pflicht_gesamt` im Overlay ist wirkungslos und durch keinen Test geschützt (Claude, Minor): Kern und DB liefern immer `len(package.required_fields())`. Der Schlüssel steht normativ im Satz und bleibt drin; als Kommentar in `api.py` festgehalten. **Nächster Schritt:** prüfbar erst, wenn schreibendes und antwortendes Paket verschiedene Feldmengen haben — heute nicht konstruierbar. **(d)** Die Kombination Writer + `PostgresStateStore` ist nirgends gefahren (alle neuen Tests nutzen `InMemoryStateStore`; beide Stores kopieren bei load/save tief, Aliasing wurde ausgeschlossen). **Nächster Schritt:** in die Gesamtverifikation von Task 16 aufnehmen. **(e)** `store.load` nach `process_turn` könnte laut Interface `None` liefern → `AttributeError` → 500 (Codex, Important). **Gemessen widerlegt:** ein `assert stand is not None` an der Stelle lief die volle Suite grün; jeder Rückgabepfad von `process_turn` setzt einen gespeicherten State voraus. Kein Fix — eine Sonderbehandlung wäre unerreichbarer, ungetesteter Code | wir selbst, bei Gelegenheit | (c) nichts, (d) Task 16 |
| **Notizen aus dem Task-14-Review (bewusst deferiert)** | Vier Befunde, jeweils gemessen und als vertretbar eingestuft — hier nachgehalten, damit sie nicht still verschwinden. **(a)** Der Plan-Guard „gebundene Zeile ist eingefroren, TP-ID weicht ab“ ist beim Rebind ersatzlos entfallen; die DDL faengt es (Freeze-Trigger weist das DELETE ab → nicht-terminal stiller Skip, terminal `ProfilWriteError`), kein Datenverlust, ueber die API zusaetzlich durch 409 abgeschirmt. **(b)** Eine ueber Mandantengrenzen kollidierende `session_id` wird als „fremder Draft“ gemeldet, obwohl der Fokus-Schritt frei ist — reine Diagnose-Ungenauigkeit, `pruefe_mandant` faengt den Fall vorher mit 409. **(c)** `_konflikt_zuletzt` waechst unbegrenzt (ein Eintrag je Session/Schritt, nie geraeumt) — in einem langlaufenden Dienst ein kleines, aber echtes Leck. **(d)** `_draft_aufraeumen` loggt weiterhin `grund=%r`; der Payload-Leak ueber Postgres' DETAIL-Zeile ist nur fuer den INSERT-Pfad belegt, auf dem DELETE-Pfad nicht reproduzierbar — deshalb dort nicht praeventiv geaendert (YAGNI), aber notiert. **(e)** `_einfrieren` frischt `erhebung_id` und `paket_version` nicht nach, obwohl es alle anderen Spalten neu schreibt (nur statisch gefunden, Zusammenhang mit K-K) | wir selbst, bei Gelegenheit | nichts heute |
